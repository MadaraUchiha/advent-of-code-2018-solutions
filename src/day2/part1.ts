...
export function day2Part1(input: string) {
  ...
  const withTwoOfAnyLetter = processedInput.filter(item => item.split('').some(letter => numberOfOccurences(item, letter) === 2)).length;
  const withThreeOfAnyLetter = processedInput.filter(item => item.split('').some(letter => numberOfOccurences(item, letter) === 3)).length;
